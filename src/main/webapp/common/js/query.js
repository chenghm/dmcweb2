...
function buttonOnclick(curEle, url, tarType){
		
	if(tarType == '0'){
		blankOpenWin(curEle, url, 'blank');
	}else if(tarType == '1'){
...
function blankOpenWin(curEle, url, tar){
...
function checkAdd(index, level) {
	var tr = document.getElementById('ec_table').lastChild.firstChild;
...
function checkDelete(index) {
	if (document.getElementById('ec_table').lastChild.firstChild == null)
	{
		return false;
	}
	var tr = document.getElementById('ec_table').lastChild.firstChild;
